...
async function loadAllData() {
  // Chaque section charge indépendamment — une erreur ne bloque pas les autres
  const load = async (path, key, renderFn, errorId) => {
    try {
      const res = await fetch(path);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      if (!Array.isArray(data)) throw new Error('Le fichier JSON doit être un tableau');
      state[key] = data;
      renderFn();
    } catch (err) {
      console.error(`Erreur chargement ${path}:`, err.message);
      showError(errorId, `Impossible de charger (${err.message}). Vérifiez le fichier JSON.`);
    }
  };

  await Promise.all([
    load('data/tools.json',   'tools',   () => { renderTools(); renderFavorites(); updateFavCount(); }, 'tools-grid'),
    load('data/blog.json',    'blog',    renderBlog,    'blog-list'),
    load('data/gallery.json', 'gallery', renderGallery, 'gallery-grid'),
  ]);
}
...
function renderTools() {
  // Normalisation de la recherche : trim, espaces multiples → 1 espace, minuscules, accents supprimés
  ...
  // Recherche tolérante : chaque mot doit apparaître quelque part dans l'outil
  ...
  const matchesQuery = (t) => {
    ...
    // Tous les mots doivent être présents (recherche ET)
