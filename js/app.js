...
function renderTools() {
  ...
  document.getElementById('tools-grid').innerHTML = paged.map(t => {
    ...
    const plan   = t.plan || (t.page ? 'gratuit' : null); // rétrocompatibilité
    ...
    // Action : fiche interne si page existe, sinon site externe
    const cardAction = t.page
      ? `onclick="window.location.href='${t.page}'"` 
      : `onclick="window.open('${t.url}','_blank')"`;

    // ── Badge et style selon le plan ──
    let planBadge   = '';
    ...
    if (plan === 'featured') {
      ...
      planBadge  = `<span class="tool-plan-badge tool-plan-badge-featured">★ Mis en avant</span>`;
    } else if (plan === 'starter') {
      cardClass  = 'tool-card tool-card-featured tool-card-plan-starter';
      planBadge  = `<span class="tool-plan-badge tool-plan-badge-starter">✓ Vérifié</span>`;
    } else if (plan === 'gratuit' && t.page) {
    ...
    return `
      <article class="${cardClass}" ${cardAction} style="${cardStyle}">
        <div class="tool-head">
          <div class="tool-ico" style="background:${col.bg}">${iconHtml}</div>
          <div style="flex:1">
            <div class="tool-name">${t.name}</div>
...
// Calcule la distance de Levenshtein entre deux chaînes (tolérance fautes)
...
// Retourne true si la requête correspond à la cible avec tolérance
function matchRecherche(query, target) {
  const q = normaliser(query);
  const t = normaliser(target);
  if (!q) return true;

  // 1. Correspondance directe (includes) — priorité
  if (t.includes(q)) return true;

  // 2. Chaque mot de la requête doit matcher un mot de la cible
  const mots = q.split(' ').filter(Boolean);
  const motsTarget = t.split(' ').filter(Boolean);

  return mots.every(mot => {
    // Correspondance directe sur un mot
    if (motsTarget.some(mt => mt.includes(mot) || mot.includes(mt))) return true;
    // Tolérance faute : distance de Levenshtein
    // Seuil adapté à la longueur du mot
