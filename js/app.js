...
function renderTools() {
  ...
  if (!filtered.length) { showEmpty('tools-grid'); renderPagination(0, 0); return; }

  // Pagination
  const total      = filtered.length;
  const totalPages = Math.ceil(total / state.toolsPerPage);
  if (state.toolsPage > totalPages) state.toolsPage = 1;
  const start  = (state.toolsPage - 1) * state.toolsPerPage;
  const paged  = filtered.slice(start, start + state.toolsPerPage);
  ...
  renderPagination(state.toolsPage, totalPages, total, start + paged.length);
}

function renderPagination(current, total, totalItems, shownEnd) {
  let el = document.getElementById('tools-pagination');
  if (!el) {
    el = document.createElement('div');
    el.id = 'tools-pagination';
    document.getElementById('tools-grid').insertAdjacentElement('afterend', el);
  }

  if (total <= 1) { el.innerHTML = ''; return; }

  const start = (current - 1) * state.toolsPerPage + 1;
  let pages = '';
  for (let i = 1; i <= total; i++) {
    if (i === 1 || i === total || (i >= current - 1 && i <= current + 1)) {
      pages += `<button class="pg-btn${i === current ? ' active' : ''}" onclick="goToPage(${i})">${i}</button>`;
  ...
  el.innerHTML = `
    <div class="pagination">
      <span class="pg-info">${start}–${shownEnd} sur ${totalItems} outils</span>
      <div class="pg-controls">
        <button class="pg-btn pg-arrow" onclick="goToPage(${current - 1})" ${current === 1 ? 'disabled' : ''}>‹</button>
        ${pages}
        <button class="pg-btn pg-arrow" onclick="goToPage(${current + 1})" ${current === total ? 'disabled' : ''}>›</button>
      </div>
    </div>`;
}
...
function renderGallery() {
  ...
  document.getElementById('gallery-grid').innerHTML = filtered.map((g, i) => {
    const isMusic = g.type === 'musique';
    const thumbStyle = isMusic
    ...
    const thumbContent = isMusic
      ...
      : `<img src="${g.thumb}" alt="${g.title}"
            ...
            loading="lazy">`;
    return `
      <article class="gallery-card" onclick="openGalleryItem(${i})" style="cursor:pointer;">
  ...
  // Stocker les items filtrés pour le lightbox
...
function handleNewsletterSignup() {
  ...
  // TODO: fetch('/api/newsletter', { method:'POST', body: JSON.stringify({ email }) })
  console.log('Newsletter signup:', email);
  input.value = '';
  showToast('✓ Inscription confirmée ! Merci ' + email.split('@')[0] + ' !');
}
