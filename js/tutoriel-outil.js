...
const OUTIL = {
  ...
  filtreType: 'tout',          // 'tout' | 'video' | 'short'
  filtreDureeMin: 0,
...
function renderIdentite() {
  ...
  if (metaDesc) metaDesc.setAttribute('content',
    `Tous les tutoriels vidéo pour apprendre et maîtriser ${o.nom}. ${o.videos.length} vidéos sélectionnées, filtrables par type et durée. Gratuit, en français.`
  );
...
function renderFiltres() {
  ...
  document.getElementById('filtre-short')?.addEventListener('click', () => setFiltreType('short'));
...
function filtrerVideos() {
  return OUTIL.outil.videos.filter(v => {
    /* Filtre type */
    if (OUTIL.filtreType === 'short' && v.type !== 'short') return false;
    if (OUTIL.filtreType === 'video' && v.type !== 'video') return false;
    /* Filtre durée (seulement si type === 'video') */
...
/* ─── CARTE VIDÉO (page profonde, plus grande) ─── */
function carteVideoHTML(v) {
  const isShort = v.type === 'short';
  return `
  <div class="outil-video-card ${isShort ? 'card-short' : ''}"
    ...
    <div class="outil-video-thumb">
      ...
      <span class="thumb-badge ${isShort ? 'badge-short' : 'badge-video'}">${isShort ? '▶ Short' : '▶ Vidéo'}</span>
