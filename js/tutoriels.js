...
const TUTO = {
  data: null,
  filtreActif: 'tout',       // 'tout' | 'video' | 'short'
  filtreDuree: null,         // { min, max } ou null
  outilActif: null,          // id de l'outil en vue dédiée
  carteOuverte: null,        // id de la carte accordéon ouverte
  modalOuverte: false,
  playerActif: null
};
...
function miniatureHTML(video, outil) {
  ...
  const badgeClass = isShort ? 'badge-short' : 'badge-video';
  const badgeLabel = isShort ? '▶ Short' : '▶ Vidéo';
  return `
  <div class="tuto-thumb" onclick="ouvrirPlayer('${video.youtubeId}','${escHTML(video.titre)}')" title="${escHTML(video.titre)}">
    <div class="tuto-thumb-img ${isShort ? 'thumb-short' : ''}">
      ...
      <span class="thumb-badge ${badgeClass}">${badgeLabel}</span>
...
/* ─── EXPORTS GLOBAUX (appelés depuis le HTML inline) ─── */
